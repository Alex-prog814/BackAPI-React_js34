...
import { registerUser, cleanErrorState, cleanStatusState } from '../store/userSlice';  
      ...
      {error ? (
        ...
        <>
          {status ? (
            <>
              <h2>{ status }</h2>
              <Button variant="outlined" size="medium" onClick={() => navigate('/login')}>Login now!</Button>
            </>
          ) : (
            <>
              {loading ? (
                ...
                <Box sx={{
                  ...
                  <Button variant="outlined" size="medium" onClick={() => dispatch(registerUser(user))}>Register</Button>
                </Box>
              )}
            </>
          )}
